...
from rdflib import Graph, ORG, FOAF
...
def configure_arg_parser() -> Namespace:
    ...
    # loadfull command
    parser_loadfull = subparsers.add_parser('loadfull',
                                            help='Loads KBO data into a local SQLite DB under the data folder.')
    parser_loadfull.add_argument('date', help='The date of the dump to use.')
    # loaddiff command
    parser_load_diff = subparsers.add_parser('loaddiff',
                                             help='Loads KBO data into a local SQLite DB under the data folder.')
    ...
    parser_sample = subparsers.add_parser('sample',
                                          help='Stop synchronizing a previously onboarded LDES collection with given alias.')
    ...
    parser_sample = subparsers.add_parser('version_sample',
                                          help='Stop synchronizing a previously onboarded LDES collection with given alias.')
    parser_sample.add_argument('enterprise_nr', help='The enterprise number to use as a sample.')
    parser_sample.add_argument('--format', help='The format to dump the data in.',
                               choices=['json', 'json-ld', 'turtle'], default='turtle')

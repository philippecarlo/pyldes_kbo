...
from testcase0 import MustTestCase0
from testcase1 import MustTestCase1
...
if not MustTestCase0().get_result():
    report += "Current Implementation doesn't contain a LDES Stream or a Tree Collection, Comform Score is 0 "
    print("Current Implementation doesn't contain a LDES Stream or a Tree Collection, Comform Score is 0 ")
else:
    ...
    report += "- Validating against MUST conformance request:<br/>"
    report += "When the current page is a tree:Node, there MUST be a property linking the current page URL to the URI " \
              "of " \
    ...
    report += "- Validating against MUST conformance request:<br/>"
    report += "Every entity linked from tree:view MUST be an entry point to retrieve all members of the " \
              "collection.<br/>"
    ...
    report += "- Validating against MUST conformance request:<br/>"
    report += "The result of the evaluation of the tree:path, is the value that MUST be compared to the " \
              "tree:value.<br/>"
    ...
    report += "When no tree:path is defined," \
              "the tree:value MUST be compared to all members’ triples that can be compared to the tree:value as " \
              "defined " \
    ...
    report += "- Validating against MUST conformance request:<br/>"
    report += "When using relations such as tree:LessThanRelation or tree:GreaterThanRelation, the time literals need " \
              "to " \

...
class KboActivity(KboBase):
    ...
    def to_rdf_version(self, graph: Graph, as_blank_node: bool = False) -> URIRef:
        ...
        # add nace code
        graph.add(
            (activity_ref, LEGAL.companyActivity, self.nace_code.to_rdf_version(graph, as_blank_node=as_blank_node)))
        # graph.add((activity_ref, KBO.naceVersion, Literal(self.nace_version)))
        graph.add(
            (activity_ref, KBO.classification, self.classification.to_rdf_version(graph, as_blank_node=as_blank_node)))
        return activity_ref
